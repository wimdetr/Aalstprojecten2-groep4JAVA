...
public class JobCoachRepository {
    ...
    public void modifyJobCoach(JobCoach j) {
        jobCoachMapper.modifyJobCoach(j);
    }
    ...
    public List<JobCoach> zoekStraat(String query) {
        return lijst.stream()
                .filter(p -> p.getStraatBedrijf().toLowerCase().startsWith(query.toLowerCase()))
                .collect(Collectors.toList());
    }

    public List<JobCoach> zoekOrganisatie(String query) {
        return lijst.stream()
                .filter(p -> p.getNaamBedrijf().toLowerCase().startsWith(query.toLowerCase()))
                .collect(Collectors.toList());
    }
    ...
    public List<JobCoach> zoekPostCode(String query) {
        return lijst.stream()
                .filter(p -> Integer.toString(p.getPostcodeBedrijf()).startsWith(query))
                .collect(Collectors.toList());
    }

    public List<JobCoach> zoekGemeente(String query) {
        return lijst.stream()
                .filter(p -> p.getGemeenteBedrijf().toLowerCase().startsWith(query.toLowerCase()))
                .collect(Collectors.toList());
    }
}
